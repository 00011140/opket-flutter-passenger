...
class GeneralApiService {
  ...
  Future<bool> verifyPassenger() async {
    try {
      ...
      final res = await _dio.get("/user/verify-passenger/$phone");
  ...
  Future<void> sendAppInfo(String? version, bool? notificationEnabled) async {
    ...
    try {
      await _dio.post("${Env.baseUrl}/user/app-version/$phone", {
  ...
  Future<void> deleteAccount() async {
    ...
    if (phone == null) return;
    try {
      await _dio.post("${Env.baseUrl}/user/delete-account", {"phone": phone});
